"""
Модуль для удаления устаревших постов из Qdrant
"""
...
class PostDeletionService:
    ...
    async def delete_expired_posts(self) -> int:
        ...
        try:
            # Получаем все документы и фильтруем те, у которых delete_date <= today
            # Qdrant не поддерживает прямое сравнение строковых дат в фильтрах,
            # поэтому получаем все документы и фильтруем в коде
            expired_post_ids = []
            offset = None
            limit = 100  # Обрабатываем по 100 документов за раз
            ...
            while True:
                ...
                # Фильтруем посты с delete_date <= today
                # Игнорируем посты без delete_date (как указано в требованиях)
                for point in points:
                    payload = point.payload or {}
                    delete_date_str = payload.get("delete_date")
                    
                    # Если у поста нет delete_date, игнорируем его
                    if not delete_date_str:
                        continue
                    
                    try:
                        # Сравниваем даты (формат YYYY-MM-DD, строковое сравнение работает)
                        if delete_date_str <= today:
                            expired_post_ids.append(str(point.id))
                    except (TypeError, ValueError) as e:
                        logger.debug(
                            f"⚠️ [post_deletion_service] Не удалось сравнить дату {delete_date_str}: {e}"
                        )
            ...
            logger.info(
                f"📊 [post_deletion_service] Найдено {len(expired_post_ids)} постов для удаления"
            )

            # Удаляем посты с retry механизмом
    ...
    async def _delete_with_retry(self, post_ids: list[str]) -> int:
        ...
        for attempt in range(1, max_retries + 1):
            if not remaining_post_ids:
                logger.info(
                    f"✅ [post_deletion_service] Все посты успешно удалены. Всего попыток: {attempt - 1}"
                )
                return total_deleted

            try:
                # Разбиваем на батчи для более надежного удаления
                batch_size = 100
                ...
                for i in range(0, len(remaining_post_ids), batch_size):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"⚠️ [post_deletion_service] Ошибка при удалении батча на попытке {attempt}: {e}"
                        )
                        # Сохраняем неудачный батч для повторной попытки
                        failed_batches.extend(batch)

                # Обновляем список оставшихся постов для следующей попытки
                remaining_post_ids = failed_batches

                if not remaining_post_ids:
                    logger.info(
                        f"✅ [post_deletion_service] Все посты успешно удалены с попытки {attempt}"
                    )
                    return total_deleted
                else:
                    logger.warning(
                    ...
                    # Если есть неудачные посты, делаем повторную попытку
