...
from tplexity.tg_parse.config import settings as tg_parse_settings
...
# Pydantic модели (для обратной совместимости с API)
...
class TelegramMonitorService:
        ...
        for channel in self.config.channels:
            try:
                ...
                if new_messages:
                    ...
                    logger.info(
                        f"Канал {channel}: найдено {len(new_messages)} новых сообщений ({len(messages_with_text)} с текстом)"
                    )
    ...
    async def _send_to_webhook(self, chunks: list[dict[str, Any]]):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(self.config.webhook_url, json={"chunks": chunks}, timeout=30.0)
    ...
    async def download_initial_messages(self) -> dict[str, Any]:
        ...
        results = {"total_downloaded": 0, "total_saved": 0, "channels": {}}
        ...
        for channel in self.config.channels:
            try:
                ...
                messages = await self.downloader.download_messages(
                    channel_username=channel, limit=self.config.initial_messages_limit
                )
        ...
        logger.info(
            f"Скачивание завершено. Всего скачано: {results['total_downloaded']}, сохранено: {results['total_saved']}"
        )
        return results
...
service: TelegramMonitorService | None = None
...
async def startup_event():
    ...
    # Загружаем конфигурацию из .env через config.py
    global config
    config = ServiceConfig(
        channels=tg_parse_settings.get_channels_list(),
        check_interval=tg_parse_settings.check_interval,
        initial_messages_limit=tg_parse_settings.initial_messages_limit,
        webhook_url=tg_parse_settings.webhook_url,
        api_id=tg_parse_settings.api_id,
        api_hash=tg_parse_settings.api_hash,
        phone=tg_parse_settings.phone,
        session_name=tg_parse_settings.session_name,
        data_dir=tg_parse_settings.data_dir,
    )
    logger.info(f"Загружена конфигурация из .env.tg_parse: {config.channels}")
...
async def start_monitoring(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {"status": "started", "channels": config.channels, "check_interval": config.check_interval}
    except Exception as e:
...
async def download_messages():
    ...
    try:
        ...
        return {"status": "success", "limit_per_channel": config.initial_messages_limit, "results": results}
